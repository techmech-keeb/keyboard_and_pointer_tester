...
Surface Pro 7 と ThinkPad X9 14インチを想定した、Edge キオスクモード向けのローカル HTML 入力テストツールです。ページがアクティブな状態で、特定の入力欄や描画領域をクリックしなくても、キーボード入力・文字入力・ポインター操作を 1 画面で可視化します。ポインター軌跡は全画面の背景レイヤーに描画し、入力テキストや操作量の数値は、近未来的な制御炉端末をイメージした Minimal Reactor Console 風の暗色HUDとして表示します（固有ロゴや直接的な意匠コピーは使用しません）。
...
- 画面全体を背景キャンバスとして使用し、連続したポインター軌跡とクリック / タップ波紋を全画面に描画
- Live Text、Keyboard Matrix、Motion Compass はキャンバスより前面の半透明HUDとして表示
- HUDの背面でもポインター軌跡が見えるよう、パネル・テキスト領域は薄いガラス調の背景にしています
- Canvas は `pointer-events: none` とし、UI操作と全画面入力検知を両立
- ポインター軌跡は一定距離・時間ごとにサンプリングし、上限点数を抑えつつ、描画時は連続パスとしてつなげて表示

### Minimal Reactor Console HUD

- チャコール〜黒の背景に、制御炉リング、薄いグリッド、青緑のポインター軌跡を重ねて表示
- Live Text、Keyboard Matrix、Motion Compass は暗色ガラスHUDとして表示し、シアンとライムのアクセントで診断端末らしく整理
- キー押下中はライム色で面発光し、押下履歴とShift側サブレジェンドはシアン / ライム系で識別しやすくしています
- 公式ロゴや固有意匠は使わず、リアクター制御端末の抽象的な雰囲気だけを取り入れています
...
- Keyboard Matrixの右側に小型の Motion Compass を統合
- `document` レベルのポインター移動量と `wheel` / タッチパッドスクロールの `deltaX` / `deltaY` を同じ方位盤上に重ねて表示
- ポインター移動は青白い実線ベクトル、スクロールは濃いブルーの破線ベクトルとして区別
- 数値表示はインジケーター枠の直下にある小型ミニreadoutへ集約し、`P x,y` と `S x,y` の必要最低限だけを低コントラストで表示
...
4. クリックまたはタップすると全画面背景レイヤーにリアクター風の波紋と十字マーカーが表示され、入力テキストや操作量の数値は半透明HUD越しに確認できます。
...
- **HTML5 Canvas**: Full-screen Pointer Canvas の連続発光軌跡・リアクター風波紋描画、Motion Compass のポインター / スクロール重ね合わせベクトル描画
