...
Surface Pro 7 と ThinkPad X9 14インチを想定した、Edge キオスクモード向けのローカル HTML 入力テストツールです。ページがアクティブな状態で、特定の入力欄や描画領域をクリックしなくても、キーボード入力・文字入力・ポインター操作を 1 画面で可視化します。ポインター軌跡は全画面の背景レイヤーに描画し、入力テキストや操作量の数値は、映画「Oblivion」に見られるような白基調・航空機コンソール風の抽象的な半透明HUDとして表示します（固有ロゴや直接的な意匠コピーは使用しません）。
...
### Oblivion-inspired Skyline Glass HUD

- 白〜ライトグレーの背景に、青白いポインター軌跡とクリック / タップ波紋を表示
- Live Text、Keyboard Matrix、Pointer Status / Input Status は薄いガラス板のような半透明HUDとして表示
- キー押下中は白く発光し、押下履歴とShift側サブレジェンドはブルー系で表示して識別しやすくしています
- 公式ロゴや固有意匠は使わず、航空機・ドローン操作端末の抽象的な雰囲気だけを取り入れています
...
- 数字・記号キーなど2つのレジェンドがある全キーで、通常入力の主レジェンドをダークグレー、Shift入力側の副レジェンドをブルーで表示します
...
### Motion / Scroll Compass

- Keyboard Matrixの右側に小型の Motion Compass と Scroll Compass を統合
- Motion Compass は `document` レベルのポインター移動量から速度ベクトルを計算し、青白い方向線と方位ラベルで表示
- Scroll Compass は `wheel` / タッチパッドスクロールの `deltaX` / `deltaY` を別インジケーターとして検知し、破線ベクトルで表示
- `vx`、`vy`、`|v|` と `sx`、`sy`、`|s|` をコンパクトな数値HUDとして表示
- 既存の全画面ポインター軌跡とは別に、現在の移動方向・スクロール方向・速度感を即時確認できます

### Pointer Status / Input Status

- Pointer Status: 入力種別、座標、移動量、ボタン状態を表示
- Input Status: 最後のキー、物理キーコード、IME状態、ホイール累積量を表示（スクロール方向は Scroll Compass にも反映）
...
4. クリックまたはタップすると全画面背景レイヤーに青白い波紋が表示され、入力テキストや操作量の数値は半透明HUD越しに確認できます。
5. ホイールまたはタッチパッドでスクロールすると、Input Status のホイール累積量と Scroll Compass の方向ベクトルに反映されます。
...
- **HTML5 Canvas**: Full-screen Pointer Canvas の青白い軌跡・波紋描画、Motion Compass / Scroll Compass のベクトル描画
...
- **Wheel Events**: `document` レベルの `wheel` でホイール / タッチパッドスクロール量を検知し、Scroll Compassにも反映
