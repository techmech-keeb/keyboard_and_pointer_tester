...
- [ ] **テーマ切替**（WebView2 で同梱 4 テーマ——スタンダード / レトロ液晶 / ミニマル /
      ポップ——がすべて意図どおり描画されるか。色面の多いポップと、細線の多いミニマルは
      特に見え方が環境に左右されやすい）
...
### 6. デザインテーマの追加（完了・実機確認だけ残る）

**2026-09-13 に完了**。[#66](https://github.com/techmech-keeb/keyboard_and_pointer_tester/pull/66) で
「ミニマル」（静かな明色）と「ポップ」（高彩度の色面）が入り、同梱テーマは 4 つになった。
依頼の経緯は [`archive/theme-addition-implementation-request.md`](archive/theme-addition-implementation-request.md)（凍結）。

この作業で変わった前提:

- `visual-check` の撮影は **180 枚**（3 サイズ × 4 テーマ × 15 状態）。CI の `screenshots` は
  2 分程度で終わるため、撮影の削減はしていない。**テーマを増やすときはここが 1 テーマ 45 枚ずつ増える**
- Canvas の線幅・グローの太さ・コンパスの透明度は `ui/app.js` に固定値で、テーマ変数化されていない。
  次にテーマを増やすか、既存テーマの質感を変えたくなったときに検討する（今回は配色のみ追従）
- 「実機確認」A のテーマ項目は **4 テーマとも見る**ことになる（項目は増えない）
