...
- **INPUT MONITOR**: クリック / スクロールのカウンタ、標準／高解像度スクロール判定とノッチ換算、直近入力のライブフィード（連続入力は ×N 集約）
...
接続した OLSK60（vial-qmk）から **Vialプロトコル標準機能のみ**で情報を取得します。ファーム独自パッチには依存しません。RMK 0.9 系とも通信プロトコルは互換ですが、RMK版プロファイル、EEPROM layout options、エンコーダー表示は未対応です。ハイレゾスクロールは受信判定とノッチ換算まで対応し、エフェクト集約と横方向表示が残っています。
