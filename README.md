...
- Input Status: 最後のキー、物理キーコード、IME状態、ホイール累積量を表示（スクロール方向は Motion Compass にも反映）
...
5. ホイールまたはタッチパッドでスクロールすると、Input Status のホイール累積量と Motion Compass の破線方向ベクトルに反映されます。
...
- **HTML5 Canvas**: Full-screen Pointer Canvas の青白い軌跡・波紋描画、Motion Compass のポインター / スクロール重ね合わせベクトル描画
