...
- Shift入力側のレジェンドはブルー、通常入力側のレジェンドはダークグレーで表示して識別しやすくしています
...
- Keyboard Matrixの右側に小型Motion Compassを統合
- `document` レベルのポインター移動量から速度ベクトルを計算し、青白い方向線と方位ラベルで表示
- `vx`、`vy`、`|v|` をコンパクトな数値HUDとして表示
- 既存の全画面ポインター軌跡とは別に、現在の移動方向と速度感を即時確認できます
...
- **HTML5 Canvas**: Full-screen Pointer Canvas の青白い軌跡・波紋描画、Motion Compassのベクトル描画
