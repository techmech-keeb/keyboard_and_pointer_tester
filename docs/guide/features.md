...
- **ダーク**（暗色 HUD、既定）、**モノクロ**（明色紙背景 + 黒インク + 等幅フォントの計測機器風）、**ミニマル**（中性グレー + 低彩度 + 細い線）、**ポップ**（明るい色面 + 高彩度6色 + 太い輪郭 + 大きい角丸）を同梱
- ポップは左クリック＝ターコイズ、中クリック＝オレンジ、右クリック＝マゼンタ、スクロール＝コバルト、コンパス＝紫、キーボード面＝イエロー。押下キーはマゼンタのベタ塗りに白文字で表示
- スタッフメニュー →「表示オプション」→「デザイン」で切替。設定は端末ごとに `localStorage` に保存（保存値は id なので、表示名が変わっても選択は残る）
