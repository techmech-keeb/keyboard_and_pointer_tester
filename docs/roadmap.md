...
- **ガイドツアーラウンド**（PR #23〜#30, merged, 2026-07-18）:
  - **ガイドツアー機能 Phase 1**: `ui/tours.js`（エンジン）+
    `ui/layouts/olsk60.tours.js`（速度プロファイル / サウンドの2ツアー）。
  ...
  - **ツアーデータの出自管理**: `olsk60.tours.js` 冒頭に出典
...
- [ ] 詳細スクロール（hi-res）の表現: 解像度インジケータ・ノッチ換算・フィードのノッチ集約・
      横スクロール方向表示（同書 §4.4、現行との差分は §9.3）。現状はサブノッチ入力も通常の
      wheel event として表示するだけで、ハイレゾとしての判定・正規化・表現は未対応
