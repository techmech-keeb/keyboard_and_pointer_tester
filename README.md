...
接続した OLSK60（vial-qmk）から **Vialプロトコル標準機能のみ**で情報を取得します。ファーム独自パッチには依存しません。RMK 0.9 系とも通信プロトコルは互換ですが、RMK版プロファイル、EEPROM layout options、エンコーダー表示、ハイレゾスクロール表現は未対応で、移行作業が残っています。
